...
const Cat = ({ id, genotype, disabled, onClick, ...props }) => {
  ...
  const { headSize, eyeShape, earSet, muzzleLength, red, tabby, point, white } =
    phenotype;
    ...
    >
      ...
      <g filter={`url(#head-shadow-${id})`}>
        <Ears
          id={id}
          earSet={earSet}
          rightColor={colors.rightEar}
          leftColor={colors.leftEar}
        />
      ...
      <g clipPath={`url(#head-${id})`}>
        <g clipPath={point === 'point' ? `url(#point-${id})` : 'none'}>
          ...
          {red === 'tortie' && tabby == 'tabby' && (
            <g clipPath={`url(#tortie-${id})`}>
              <Tabby color={colors.tortieStripes} />
            </g>
          )}
        </g>
